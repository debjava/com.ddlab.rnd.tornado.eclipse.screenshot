...
public class PluginUtil {
	...
	public static Image getScreenShotImage() {
		Image image = new Image(Display.getCurrent(),
				ScreenShotImages.class.getResourceAsStream("camera-icon16.png")); // sc16.png
		return image;
	}

	public static Image getSplitImage() {
		Image image = new Image(Display.getCurrent(),
				ScreenShotImages.class.getResourceAsStream("editorspilit.png"));
		return image;
	}
...
//			Rectangle disrect = display.getClientArea();
			...
			Rectangle client = display.getActiveShell().getBounds();
...
//					.getActiveWorkbenchWindow()).getStatusLineManager();
			...
			Image image = new Image(display, client);
	...
	public static void saveButtonScreenShot() {
		Display display = Display.getDefault();
		// Display display = Display.getCurrent();
		GC gc = new GC(display);
		// Rectangle disrect = display.getClientArea();
		// Rectangle client = display.getActiveShell().getClientArea();
		Rectangle client = display.getActiveShell().getBounds(); // Correct
